import { updateReadmeContents } from '.';
...
1. Run \`npx @kaiosilveira/refactoring-catalog-cli prepare-repository -r <repo_name>\`. It will:
...
2. Update repo name and URL at \`package.json\`
...
describe('updateReadmeContents', () => {
  it('should update the CI badge with the actual repo name', () => {
    const repoName = 'rename-field-refactoring';
    const updatedMarkdown = updateReadmeContents(markdown, repoName);
    const badge = updatedMarkdown.split('\n')[0];
    expect(badge).toContain(repoName);
  });

  it('should update the link to the commit history tab with the actual repo name', () => {
    const repoName = 'rename-field-refactoring';
    const updatedMarkdown = updateReadmeContents(markdown, repoName);
    const commitHistoryLink = updatedMarkdown.split('\n').pop();
    expect(commitHistoryLink).toContain(repoName);
  });

  it('should replace "Refactoring name" with the actual humanized repo name', () => {
    const repoName = 'rename-field-refactoring';
    const updatedMarkdown = updateReadmeContents(markdown, repoName);
    expect(updatedMarkdown).toContain('# Rename Field');
  });
});
